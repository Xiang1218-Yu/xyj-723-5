...
const DefinesLineCapsMapping: Record<number, LineCaps> = Object.keys(LineCapsDefinesMapping).reduce(
    (r, lineCapsName) => {
...
const DefinesLineDashesMapping: Record<number, LineDashes> = Object.keys(
    ...
    implements DisplacementFeature, FadingFeature
{
    ...
    constructor(params?: SolidLineMaterialParameters) {
        ...
        // Optional displacement texture derived from the incoming parameters.
        ...
        if (params) {
            if (params.color !== undefined) {
                // `number | string` is a valid subset of THREE.ColorRepresentation.
